...
class Trainer():
            ...
            for idx in range(len(train_loader)):
                ...
                if (idx + 1) % log_interval == 0:
                    ...
                    eta = datetime.timedelta(seconds=int(
                        seconds_per_batch * (len(train_loader) - idx - 1) + seconds_per_batch * len(train_loader) * (
                                    total_epoch - epoch - 1)))
                    logger.info(
    ...
    def eval_classifier(self, data_loader, prefix, save=False):
        ...
        for idx, input_data in tqdm.tqdm(enumerate(data_loader)):
            end = time.time()
            X, targets,imgPath = self.get_input(input_data)
            # measure data loading time
            data_time.update(time.time() - end)
