...
class Evaluator():
    ...
    def classify(self, probs, targets, test_data, multilabel=False,imgPathNameList=None):
        """
        ...
        if multilabel:
            self._eval_multilabel(probs, targets, test_data,imgPathNameList)
        else:
            self._eval_singlelabel(probs, targets, test_data,imgPathNameList)
    ...
    def _eval_singlelabel(
        ...
        if number of labels == 2:
            top1 and rocauc
        """
        acc_dict = singlelabel.compute_acc_auc(scores, targets,imgpaths)
