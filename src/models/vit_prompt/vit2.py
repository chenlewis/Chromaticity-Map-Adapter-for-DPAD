...
class PromptedTransformer(Transformer):
    def __init__(self, prompt_config, config, img_size, vis):
        ...
        # 使用全局平均池化层
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)  # 输出大小为 (batch_size, 64, 1, 1)

        # 使用线性层进行中间映射
        self.linear1 = nn.Linear(64, prompt_dim)  # 注意这里的输入维度变为 64

        # 使用卷积层进行升维
        self.conv3 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
        self.conv4 = nn.Conv2d(32, 10, kernel_size=3, stride=1, padding=1)
    ...
    def IIC_batch(self, batch_x):
        # batch_x 是一个包含批次中每个图像的张量，其维度为 [batch_size, C, H, W]
        batch_y = []
        ...
        # 将 batch_y 转换为张量，其维度为 [batch_size, C, H, W]
        batch_y_tensor = torch.stack(batch_y, dim=0)
        batch_y_tensor = batch_y_tensor.permute(0, 2, 1, 3)
        return batch_y_tensor

    def fft(self, x):
        # Assuming x is a real tensor with shape [batch_size, H, W]
        mask = torch.zeros(x.shape, device=x.device)
        w, h = x.shape[-2:]
        line = int((w * h * 0.25) ** .5 // 2)
    ...
    def fft_batch(self, batch_x):
        batch_y = []

        for x in batch_x:
            fft_result = self.fft(x)
            batch_y.append(fft_result)
    ...
    def incorporate_prompt(self, x, y):
        ...
        y = self.conv1(y)
        y = self.conv2(y)#64,56,56

        y = self.global_avg_pool(y)#64,56,56
        y = y.view(y.size(0), -1)#64
        y = self.linear1(y)#[b,768]
        # print(y.shape)

        prompt_embeddings_tensor = y.unsqueeze(1).expand(-1, self.num_tokens, -1)#[B,10,768]
        # print(prompt_embeddings_tensor.shape)
        prompt_embeddings = nn.Parameter(prompt_embeddings_tensor)
    ...
    #         # combine prompt embeddings with image-patch embeddings
    #         B = x.shape[0]
    #         # print("embedding before",x.shape)
    #         # after CLS token, all before image patches
    #         x = self.embeddings(x)  # (batch_size, 1 + n_patches, hidden_dim)  (n,197,768)
    #         x = torch.cat((
    #                 x[:, :1, :],
    #                 self.prompt_dropout(self.prompt_proj(self.prompt_embeddings).expand(B, -1, -1)),
    #                 x[:, 1:, :]
    #             ), dim=1)
    #         # print("embedding after",x.shape)
