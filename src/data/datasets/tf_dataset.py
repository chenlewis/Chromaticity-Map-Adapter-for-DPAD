...
tf.compat.v1.config.experimental.set_visible_devices([], 'GPU')  # set tensorflow to not use gpu  # noqa
DATASETS = [
    'caltech101',
...
def preprocess_fn(data, size=224, input_range=(0.0, 1.0)):
    image = data["image"]
    image = tf.compat.v1.image.resize(image, [size, size])

    image = tf.compat.v1.cast(image, tf.compat.v1.float32) / 255.0
