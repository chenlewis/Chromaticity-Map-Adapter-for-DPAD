...
class JSONDataset(torch.utils.data.Dataset):
    ...
    def _construct_imdb(self, cfg):
        ...
        anno = self.get_anno()
        # print(list(anno.keys()), len(list(anno.keys())))
    ...
    def get_class_weights(self, weight_type):
        ...
        if "train" not in self._split and "test" not in self._split and "val" not in self._split:
            raise ValueError(
...
#####finetune by myself
class Doc1Dataset(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(Doc1Dataset, self).__init__(cfg, split)

    def get_imagedir(self):
        return os.path.join(self.data_dir, "images")


class Doc2Dataset(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(Doc2Dataset, self).__init__(cfg, split)

    def get_imagedir(self):
        return os.path.join(self.data_dir, "images")

class S1(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(S1, self).__init__(cfg, split)

    def get_imagedir(self):
        return os.path.join(self.data_dir, "images")


class S2(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(S2, self).__init__(cfg, split)

    def get_imagedir(self):
        return os.path.join(self.data_dir, "images")

class S3(JSONDataset):
    """Nabirds dataset."""

    def __init__(self, cfg, split):
        super(S3, self).__init__(cfg, split)
