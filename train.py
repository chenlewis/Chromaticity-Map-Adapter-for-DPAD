...
def main(args):
    ...
    model, cur_device = build_model(cfg)
    trainer = Trainer(cfg, model, evaluator, cur_device)  # Initialize the trainer

    train_loader, val_loader, test_loader = get_loaders(cfg, logger)  # Get data loaders

    # Call the trainer's train_classifier method for training
    trainer.train_classifier(train_loader, val_loader, test_loader)

    # Call the trainer's eval_classifier method for evaluation
